...
2. **What did it cost?** Iterations, dollars from the provider dashboard, and wall-clock time. Post the numbers next to the cost cap from your post last night.
3. **What would you tighten?** If you ran the same loop again tonight, what's the one check you'd add to the verifier?

## Note for the host

The verifiers that failed overnight are the most interesting data points. A loop that shipped a clean PR confirms what we already believe; a verifier that passed on garbage, or never passed at all, is new information. Collect the failures — cross-reference against last night's channel posts, and fold the good ones into `verifiers/anti-patterns.md` for the next event.
